...
void f_add_move(int x, int y, MoveTab *moves, int *possible_moves, Pion *game_board, int player) {
    for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            Pion *test_board = f_init_plateau();
            f_copie_plateau(game_board, test_board);
            if (0 == f_test_mouvement(test_board, x, y, x + deplX, y + deplY, player)) {
